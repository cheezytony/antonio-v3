  ...
  for (let i = 0; i < 7; i++) {
    ...
    const newRgb = {
      r: Math.min(
        ...
        Math.max(
          0,
          rgb.r * (shouldLighten ? 1 + brightnessAdjust : 1 - brightnessAdjust),
        ),
      ),
      g: Math.min(
        255,
        Math.max(
          0,
          rgb.g * (shouldLighten ? 1 + brightnessAdjust : 1 - brightnessAdjust),
        ),
      ),
      b: Math.min(
        255,
        Math.max(
          0,
          rgb.b * (shouldLighten ? 1 + brightnessAdjust : 1 - brightnessAdjust),
        ),
      ),
    };
